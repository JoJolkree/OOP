...
namespace Delegates.Reports
{
    public abstract class ReportMaker
    {
        ...
        public string MakeReport(IEnumerable<Measurement> measurements)
        {
            ...
            result.Append(MakeItem("Temperature", MakeStatistics(measurements.Select(z => z.Temperature)).ToString()));
            result.Append(MakeItem("Humidity", MakeStatistics(measurements.Select(z => z.Humidity)).ToString()));
            result.Append(EndList());
            return result.ToString();
        }
    }

    public class ReportMaker1 : ReportMaker
    {
    ...
    public class ReportMaker2 : ReportMaker
    {
        protected override string Caption
        {
            get
    ...
    //    {
    //        return new ReportMaker1().MakeReport(data);
    //    }

    //    public static string MakeSecondReport(IEnumerable<Measurement> data)
    //    {
    //        return new ReportMaker2().MakeReport(data);
    //    }
